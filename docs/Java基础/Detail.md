...
![image-20210623184723896](pics/image-20210623184723896.png)

![image-20210623184740393](pics/image-20210623184740393.png)
...
- **`main`方法中调用`start()`方法启动线程**（从`main`线程启动一个子线程），**而不是直接调用`run`方法**（`run`方法只是普通方法，么有真正地启动一个线程，会把`run`方法执行完毕才向下执行）
